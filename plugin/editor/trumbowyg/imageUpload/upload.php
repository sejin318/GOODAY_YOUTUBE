...
if (!function_exists('trum_reprocessImage')) {
    function trum_reprocessImage($file_path, $callback) {
...
if ($get_nonce && ft_nonce_is_valid($get_nonce, 'trumbowyg')) {
...
$filename = trum_replace_filename($filename);
...
if (TRUM_UPLOAD_IMG_CHECK && ! trum_reprocessImage($savefile, null)) {
	$filesize = 0;
	$random_name = '-ERR';
	unlink($savefile);
}
...
echo json_encode(array('data' => array('link' => SAVE_URL.'/'.$filename), 'success' => true, 'status' => 200));
?>