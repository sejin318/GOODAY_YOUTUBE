...
*/
add_stylesheet('<link rel="stylesheet" href="' . BP_CSS . '/boards/board.card_pro.css">', 20);
include_once G5_LIB_PATH . '/thumbnail.lib.php';
$board_cols = 12 / $board['bb_gallery_col'];
?>
<!--######################### 목록 시작#########################-->
<div class="board-cardpro-list-wrap mb-4 mt-4">
    <div class='row'>
        <?php
        for ($i = 0; $i < count($list); $i++) {
            ...
            if ($thumb['src']) {
                ...
                $img_content = "<img class='card-img-top list-image {$lazy_load} img-fluid' {$layz_src}src='{$thumb['src']}' alt='{$thumb['alt']}' width='{$board['bo_mobile_gallery_width']}' height='{$board['bo_mobile_gallery_height']}'>";
            } else {
        ...
        ?>
            <div class='col-sm-12 col-md-<?php echo $board_cols ?> col-lg-<?php echo $board_cols ?> mb-2'>
                <div class="board-article shadow mb-3">
                    <div class="card">
                        <figure class="board-image-wrap position-relative">
                            <?php if ($is_checkbox) { ?>
                            ...
                            <?php if ($is_category && $list[$i]['ca_name']) { ?>
                                <div class='category-wrap position-absolute'>
                            ...
                            if ($is_member) {
                                //스크랩 체크
                                $scrap_class = bp_scrap_check($bo_table, $list[$i]['wr_id']);
                            ?>
                                <div class='scrap-wrap position-absolute'>
                                    <a href='#scrap-item' class='scrap-this-item <?php echo $scrap_class ?>' data-bo_table='<?php echo $bo_table ?>' data-wr_id='<?php echo $list[$i]['wr_id'] ?>'><i class="fa fa-bookmark" aria-hidden="true"></i></a>
                                </div>
                            <?php } ?>
                        ...
                        <div class='board-contents-wrap card-body'>
                            <h5 class='card-title board-subject text-cut'>
                                <?php
                                if ($list[$i]['is_notice']) {
                                    // 공지사항
                        ...
                        <div class='card-footer position-relative'>
                            ...
                            <?php echo "<div class='text-datetime position-absolute'>{$list[$i]['datetime2']}</div>"; ?>
                        </div>
                    </div>
                    <!--//.board-content-wrap-->
                </div>
...
<script>
    $(function() {
        $('.scrap-this-item').click(function() {
            var thisItem = $(this);
            var this_bo_table = $(this).data('bo_table');
            var this_wr_id = $(this).data('wr_id');

            //처리 스크립트
            $.ajax({
                url: '<?php echo G5_PLUGIN_URL . DS . "ask-favorite" . DS . "toggle_scrap.php"; ?>',
                dataType: 'json',
                method: 'POST',
                data: {
                    wr_id: this_wr_id,
                    bo_table: this_bo_table
                }
            }).done(function(xhr) {
                //console.log(xhr.result);
                //스크랩상태
                if (xhr.result == 'add') {
                    thisItem.addClass('scraped');
                }
                //삭제
                if (xhr.result === 'delete') {
                    thisItem.removeClass('scraped');
                }
            });
