...
<!-- 설문조사 결과 시작 { -->
<div id="poll_result" class="poll-wrap">
    <h1 id="win_title" class="page-title border-bottom border-color-6 p-2"><?php echo $g5['title'] ?></h1>
        ...
        <?php if ($is_etc) {  ?>
            <section id="poll_result_cmt">
                ...
                <?php for ($i = 0; $i < count($list2); $i++) {  ?>
                    <article class="border border-color-6 p-2">
                        <header>
                            ...
                            <span class="poll_cmt_del">
                                <?php if ($list2[$i]['del']) {
                                    echo $list2[$i]['del'] . "<i class=\"fa fa-trash-o\" aria-hidden=\"true\"></i><span class=\"sr-only\">삭제</span></a>";
                                }  ?>
                            </span>
                        </header>
                        <p>
                ...
                <?php if ($member['mb_level'] >= $po['po_level']) {  ?>
                    <form name="fpollresult" action="./poll_etc_update.php" onsubmit="return fpollresult_submit(this);" method="post" autocomplete="off" id="poll_other_q">
                        ...
                        <div id="poll_result_wcmt">
                            <h3><span class="">기타의견</span><?php echo $po_etc ?></h3>
                            <div>
                                <label for="pc_idea" class="sound_only">의견<strong>필수</strong></label>
                                <input type="text" id="pc_idea" name="pc_idea" required class="form-control required" size="47" maxlength="100" placeholder="의견을 입력해주세요">
                            </div>
                        </div>
                        <?php if (!$is_guest) {  ?>
                            <div class="form-group">
                                <div class="input-group">
                                    <input type="text" name="pc_name" id="pc_name" required class="form-control required" size="20" placeholder="이름">
                                </div>
                            </div>
                        <?php }  ?>
                        <?php echo captcha_html(); ?>
                        <div class='form-group'>
                            <button type="submit" class="btn btn-primary w-100">의견남기기</button>
                        </div>
        ...
        <aside id="poll_result_oth">
            ...
            <ul>
                <?php for ($i = 0; $i < count($list3); $i++) {  ?>
                    <li class="background-7"><a href="./poll_result.php?po_id=<?php echo $list3[$i]['po_id'] ?>&amp;skin_dir=<?php echo urlencode($skin_dir); ?>"> <?php echo $list3[$i]['subject'] ?> </a><span><i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo $list3[$i]['date'] ?></span></li>
                <?php }  ?>
            </ul>
        </aside>
        <!-- } 설문조사 다른 결과 보기 끝 -->
