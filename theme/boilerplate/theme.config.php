...
//테마 목록, 미리보기 에러 처리
if ($config['cf_theme'] == 'boilerplate' || stristr($_SERVER['PHP_SELF'], 'theme_preview.php')) {
    //테마 라이브러리 로딩
    $_theme_libs = array();
    $_lib_path = G5_THEME_PATH . DIRECTORY_SEPARATOR . '__lib';
    $tmp = dir($_lib_path);
    ...
    if (!empty($_theme_libs) && is_array($_theme_libs)) {
        natsort($_theme_libs);

        foreach ($_theme_libs as $file) {
            include_once $_lib_path . DIRECTORY_SEPARATOR . $file;
        }
        unset($file);
    }
    unset($_theme_libs);
